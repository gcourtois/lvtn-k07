...
public class FieldFormat {
	...
	public FieldFormat(String nameVal, String pictureVal, boolean bwz,
			boolean just, boolean signSep, boolean trail) {
		sym = new FieldFormatData();
		sym.type = FieldFormat.verifyCobolPicture(pictureVal);
		...
		setPicture(pictureVal);
		this.setBlankWhenZero(bwz);
		this.setJustifiedRight(just);
		this.setSignSeperate(signSep);
		this.setSignTrailing(trail);
		getJavaFormat(pictureVal);
	}
	...
	private String suppressLeadingZeros(String num) {
		...
		if (num.trim().length() > 0) {
			if (num.length() == getJavaFormat().length) {// Is this check always
															// true for correct
															// format?
	...
	public String doSimpleInsertion(String text) {
		...
		if (getNumberFormat() != null) {
			int j = 0;
			String retVal = "";
			if (text.length() > getJavaFormat().length)
				reportError();
	...
	private String doScaling(String num) {
		...
		do {
			if (num.charAt(i) == 'S')
				retVal += num.charAt(i);
			else if (num.charAt(i) == 'P' && !leftDecimal) {
				retVal += ".0";
				num = num.replace(".", "");
				leftDecimal = true;
			} else if (num.charAt(i) == 'P')
				retVal += '0';
			else
				break;
			++i;
		...
		retVal = "";
		int j = num.length() - 1;
		boolean isTrailing = !leftDecimal;
		while (j >= 0) {
			if (num.charAt(j) == 'P' && isTrailing) {
				retVal += '0';
				num.replace(".", "");
			} else if (num.charAt(j) == 'P')
				;
			else {
				retVal += num.charAt(j);
				isTrailing = false;
	...
	public String format(double d) {
		...
		else if (sym.type == Constants.BIGDECIMAL) {
	...
	public String format(BigDecimal d) {
		...
		if (sym != null) {
			if (getNumberFormat() != null)
				return suppressLeadingZeros(addSign(
						getNumberFormat()
								.format(d
										.abs()
										.setScale(
			...
			if (getNumberFormat() != null)
				return suppressLeadingZeros(addSign(
						getNumberFormat().format(
								(Math.abs(d) * Math.pow(10, scale))), (d < 0)));
			else if (sym.type == Constants.INTEGER
					|| sym.type == Constants.SHORT) {
				return new Integer(d).toString().trim();
			} else if (sym.type == Constants.STRING) {
				/*
				 * String str = String.valueOf(Math.abs(d)); for(int
				 * i=sym.maxStringLength-str.length()-((d<0)?1:0);i>0;--i)
				 * str='0'+str; str=(d<0)?"-":""+str;
				 */
	...
	public String format(long val) {
		...
		if (sym != null) {
			if (sym.type == Constants.LONG || sym.type == Constants.SHORT
					|| sym.type == Constants.INTEGER) {
				if (getNumberFormat() != null)
					return suppressLeadingZeros(addSign(getNumberFormat()
							.format((Math.abs(val) * Math.pow(10, scale))),
							(val < 0)));
				else
	...
	private static String normalizeNumberString(String str) {
		str = str.replace(" ", "").trim().replace("+", "").replace("$", "")
				.replace("*", "");
		str = str.replace("CR", "").replace("DB", "").replace(",", "")
				.replaceAll("[a-zA-Z]", "");
		...
		if (picNo >= Constants.SHORT && picNo <= Constants.BIGDECIMAL) {
			do {
				if ((i = myFormat.indexOf('(', prev)) > 0) {
					...
					if (j <= 0)
						reportError();
					int k = Integer.parseInt(myFormat.substring(i + 1, i + 1
							+ j));
			...
			this.sym.isSigned = (s.charAt(0) == 'S' || s.indexOf('+') >= 0
					|| s.indexOf('-') >= 0 || s.indexOf("CR") >= 0 || s
					.indexOf("DB") >= 0);
			...
			if (getDataCategory() != Constants.EXTERNAL_FLOATING_POINT) {
				boolean haveSign = false, haveDollar = false;
				String ret2 = "";
				char c = ' ';
			...
			if (isExponent) {
				ret = ret.substring(0, e + 1)
						+ javaFormatStr.substring(e + 1).replace('+', '0')
								.replace('-', '0');
			}

			ret = ret.replace('Z', '0').replace('*', '0').replace('#', '0')
			...
			do {
				if (i == 0) {
					...
					for (j = i; j < myFormat.length(); ++j) {
						if (myFormat.charAt(j) >= '0'
								&& myFormat.charAt(j) <= '9')
							l = l * 10 + myFormat.charAt(j) - '0';
						else if (myFormat.charAt(j) == ')')
							break;
						else
							reportError();
					}
					...
					if (myFormat.charAt(j) == ')') {
						...
						reportError();
				} else if (field.picChar == myFormat.charAt(i)) {
					if (field.picChar == 'G')
						field.len += 2;
					else
						field.len++;
